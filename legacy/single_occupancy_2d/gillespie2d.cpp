...
namespace Gillespie2D {
	...
	********************/
	...
	std::pair<bool,Site> Lattice::get_random_free_site() {
		...
		Site s(randI(1,this->_box_length),randI(1,this->_box_length));
		...
		while (occ) {
			...
			if (nbrs.size() > 0) {
				...
				// New site
				s = Site(randI(1,this->_box_length),randI(1,this->_box_length));
	...
	********************/
	...
	std::pair<bool,Site> Lattice::find_site(std::string species) {
		...
		while (it->second != species) {
			...
			if (it == this->_map.end()) {
				// Fail
				return std::make_pair(false,Site(0,0));
			};
		};
	...
	********************/
	...
	void Lattice::populate_lattice(std::map<std::string,int> counts) {
		std::pair<bool,Site> ret(true,Site(0,0));
	...
	********************/
	...
	void Lattice::write_to_file(std::string fname) 
	{
		...
		ite--;
		for (auto it = this->_map.begin(); it != this->_map.end(); it++) {
			f << it->first.x << " " << it->first.y << " " << it->second;
			if (it != ite) {
				f << "\n";
			};
		};
