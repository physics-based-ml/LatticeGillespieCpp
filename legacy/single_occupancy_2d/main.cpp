...
// Get the header
#include "gillespie2d.hpp"

using namespace Gillespie2D;
...
int main() {
	...
	// Effective square, length
	int square_area_eff = counts0["A2"] / a2;
	double square_len_eff = sqrt(square_area_eff);

	// Quantities of species to keep constant
	counts0["A1"] = a1 * square_area_eff;
	counts0["A4"] = a4 * square_area_eff;
	counts0["A5"] = a5 * square_area_eff;

	// X,Y,Z concentrations
	double x0 = 10.0, y0 = 80.0, z0 = 0.1;

	// X,Y,Z quantities
	counts0["X"] = x0*square_area_eff;
	counts0["Y"] = y0*square_area_eff;
	counts0["Z"] = z0*square_area_eff;

	// Box length
	// Total of 14662 particles - min size is 122
	int square_length = 300;

	// Times
	double dt = 0.00001;

	// Effective diffusion step, in terms of square length
	double square_step_eff = square_len_eff/square_length;
	double deff = square_step_eff / (dt * dt);
	...
	// Scale by square_step_effective^3
	std::map<std::string,double> rxn_probs;
	double aint = 1.2E-9; // Interaction area
	rxn_probs["rxnP1"] = sqrt(M_PI) * pow(square_step_eff,3) * kp1 / (8.0 * sqrt(deff) * aint);
	rxn_probs["rxnM1"] = sqrt(M_PI) * pow(square_step_eff,3) * km1 / (8.0 * sqrt(deff) * aint);
	rxn_probs["rxnP2"] = sqrt(M_PI) * pow(square_step_eff,3) * kp2 / (8.0 * sqrt(deff) * aint);
	rxn_probs["rxnM2"] = sqrt(M_PI) * pow(square_step_eff,3) * km2 / (8.0 * sqrt(deff) * aint);
	rxn_probs["rxnP3"] = sqrt(M_PI) * pow(square_step_eff,3) * kp3 / (8.0 * sqrt(deff) * aint);
	rxn_probs["rxnP4"] = sqrt(M_PI) * pow(square_step_eff,3) * kp4 / (8.0 * sqrt(deff) * aint);
	rxn_probs["rxnP5"] = sqrt(M_PI) * pow(square_step_eff,3) * kp5 / (8.0 * sqrt(deff) * aint);
	rxn_probs["rxnM5"] = sqrt(M_PI) * pow(square_step_eff,3) * km5 / (8.0 * sqrt(deff) * aint);
